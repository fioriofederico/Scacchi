...
class Cavallo(Pezzo):
    ...
    def verifica_mossa(self, destinazione):
        """
        verifica se la Cavallo può essere mosso alla destinazione
        ...
        if super().verifica_mossa(destinazione):  # le condizioni generiche sono verificate
            if self.posizione[0] == destinazione[0]:  # la mossa è lungo la stessa colonna
                ...
                print(f'La mossa {self.posizione[0]}{self.posizione[1]}, {destinazione[0]}{destinazione[1]} non è legale per la Cavallo')
                return False
        else:
            print(f'La mossa {self.posizione[0]}{self.posizione[1]}, {destinazione[0]}{destinazione[1]} non è legale per la Cavallo')
            return False
